...
class DemoApp {
     ...
     * @throws Exception
     */
    static createTxnTokenwithRequiredParams(params,callback) {
        ...
        try {
            ...
            var txnAmount =  Paytm.Money.constructWithCurrencyAndValue(Paytm.EnumCurrency.INR, params.orderAmount);


            var userInfo = new Paytm.UserInfo(params.PhoneNumber);
            userInfo.setAddress(params.userAddress);
            userInfo.setMobile(params.PhoneNumber);
            userInfo.setPincode("515001");


            // var userInfo = new Paytm.UserInfo(params.donarMobileNumer);
            // userInfo.setAddress(params.donarAddress);
            // userInfo.setEmail(params.donarEmailID);
            // userInfo.setFirstName(params.donarName);
            // userInfo.setMobile(params.donerMobileNumer);
            // userInfo.setPincode("515001");
         ...
         //   var userInfo = _SampleData.SampleData.getUserInfo();
          ...
          // let callbackUrl = "https://securegw-stage.paytm.in/theia/paytmCallback?ORDER_ID="+params.orderId
           //let callbackUrl = "https://securegw.paytm.in/theia/paytmCallback?ORDER_ID="+params.orderId
            ...
            */
            return Paytm.Payment.createTxnToken(paymentDetail).then(function (response) {
                ...
                if (response instanceof Paytm.SDKResponse) {
                    ...
                    var txnValue =response.getJsonResponse();
                    return callback({
                        status:3,
                        data : {
                            message :"Token Generated sucessfully ...",
                            Tokenval :JSON.parse(txnValue)
                        }
                    })
                }
                // DEBUGGING INFO
     ...
     * @throws Exception
     */
   static getPaymentStatus(params,callback) {
        //console.log("\n\ngetPaymentStatus\n");
        try {
            /** ..... Merchants code here .... */
            ...
            return Paytm.Payment.getPaymentStatus(paymentStatusDetail).then(function (response) {
                // var statResult = JSON.parse(response.getJsonResponse);
                // return statResult;
                if (response instanceof Paytm.SDKResponse) {
                    ////console.log("\nRaw Response:\n", response.getJsonResponse());
                     var txnDataJSON = JSON.parse(response.getJsonResponse())
                    return callback({
                        status:3,
                        txnData : txnDataJSON.body
                    })
                    
                    //var txnValue =response.getJsonResponse();
                    //return JSON.parse(txnValue);
                }
                // DEBUGGING INFO
                //console.log("\nRESPONSE RECEIVED IN DEMOAPP: ", response.getResponseObject());
                return callback({
                    ...
                    data : {
                        ...
                        txnData:response.getResponseObject()
                    }
                })
                // DEBUGGING INFO ENDS
            });
